...
/**
 Marks as deleted the document with the specified ID, preserving the existing properties and adding additional properties generated by a block.
 
 @param documentID The document ID.
 @param block A block executed on the proposed revision that will create additional properties that should be applied to the revision.
 @return A signal that completes when the document is deleted.
 @discussion This is similar to -[CBLDatabase (ReactiveCouchbaseLite) rcl_deleteDocumentWithID:] in that it marks the
 document as deleted.  However, it retains all existing properties.
 */
